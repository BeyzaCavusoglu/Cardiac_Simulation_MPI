...
//  1d MPI ++ OPENMP IMPLEMENTATION FUNCTIONS:
...
//  cout <<"BURASI OPENMP + MPI 1D VERSIYON"<<endl;
  ...
  int rank_north = rank - 1; //size_X + 2 remember
  ...
  if(rank != (numtasks - 1))
    ...
    if (plot_freq){
      ...
      if ((t - k * plot_freq) < dt){
	      //splot(E,t,niter,m+2,n+2); //closed for the performance tests
      }
    }
  }//end of while loop
