...
// 1D MPI IMPLEMENTATION FUNCTIONS:
...
void print2D(double** array, int rows, int cols) {
 ...
 /*  if(rank==1){
  ...
  if(rank != (num_processes - 1))
   ...
   MPI_Waitall(request, arr_request, arr_status);
    ...
    double **tmp = local_E; local_E = local_E_prev; local_E_prev = tmp;


 // COMMENTING THE PLOT FOR THE PERFORMANCE TESTS
    if (plot_freq){
  ...
  if(rank == 0) 
  {
    ...
    cout << "Max: " << mx <<  " L2norm: "<< l2norm << "rank:" << rank << endl; 
  }
